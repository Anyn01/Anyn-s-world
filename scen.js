...
class SceneEngine {
  ...
  displayNextLine() {
    ...
    if (line.startsWith("*choice")) {
      ...
      while (this.lines[this.index + 1]?.startsWith("#")) {
        const choiceLine = this.lines[this.index + 1].substring(1).trim();
        const resultLine = this.lines[this.index + 2]?.trim();
      ...
      this.index++; // Move past the final choice header line
      return;
    }
